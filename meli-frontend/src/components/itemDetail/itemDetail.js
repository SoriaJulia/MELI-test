        ...
        item && <Container>
            ...
            <Row >
                ...
                <Col className="detail" xs='3'>
                    <div className='condition'>{`${conditions[item.condition]} - ${item.sold_quantity} vendidos`}</div>
                    <div className='title'>{item.title}</div>
                    <div className='price'>{formatPrice(item.price.currency, item.price.amount)}</div>
                    <div className="d-grid gap-2"><Button variant="primary" size="lg">Comprar</Button></div>
                </Col>
            </Row>
